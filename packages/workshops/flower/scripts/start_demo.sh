...
sed "s|docker run|docker run --name $SUPERNODE1_NAME|" "$RYZERS_ROOT/ryzers.run.${SUPERNODE1_NAME}.sh" > "$TEMP_SUPERNODE1"
...
sed "s|docker run|docker run --name $SUPERNODE2_NAME|" "$RYZERS_ROOT/ryzers.run.${SUPERNODE2_NAME}.sh" > "$TEMP_SUPERNODE2"
...
sed "s|docker run|docker run --name $SUPEREXEC_SERVER_NAME|" "$RYZERS_ROOT/ryzers.run.${SUPEREXEC_SERVER_NAME}.sh" > "$TEMP_SERVER"
chmod +x "$TEMP_SERVER"
...
sed "s|docker run|docker run --name $SUPEREXEC_CLIENT1_NAME|" "$RYZERS_ROOT/ryzers.run.${SUPEREXEC_CLIENT1_NAME}.sh" > "$TEMP_CLIENT1"
...
sed "s|docker run|docker run --name $SUPEREXEC_CLIENT2_NAME|" "$RYZERS_ROOT/ryzers.run.${SUPEREXEC_CLIENT2_NAME}.sh" > "$TEMP_CLIENT2"
